...
import { debounce } from 'vue-debounce'
  ...
  const searchBreed = ref("");  
    ...
    const { data: breedImage, status, error: breedImageError, refresh, pending } = useFetch(
...
<template>
  <div class=" flex flex-col gap-8 justify-center items-center w-full py-4 p-4" >
    <div class="relative bg-gray-200 p-10 md:p-20 w-full rounded-xl flex flex-col justify-center items-center gap-4">
      <h2 class="text-black text-lg md:text-3xl font-bold capitalize font-sans">Find dog breed 🐶</h2>
      <input type="text" data-test="dog-input" v-model="searchBreed" class="border text-center rounded-md p-2"/>
      ...
      <Transition name="fade">
        <div v-if="!breedImageError && currentBreed && breedImage" class="space-y-4 my-8">
          <p class="text-center text-xl">A <span class="font-semibold" data-test="dog-selected">{{ currentBreed }}</span> !</p>
          <Doggo :src="breedImage" />
        </div>

        <div class="text-gray-500 my-6   flex justify-center items-center" data-test="dog-selected-not-found" v-else>
        Not found, try another bread :/
        </div>
      </Transition>
    </div>
   <div class="px-8">
    <div v-if="breeds && breeds.length" class="flex flex-wrap overflow-x-auto mx-auto w-[300px] md:w-full h-48 md:h-auto">
